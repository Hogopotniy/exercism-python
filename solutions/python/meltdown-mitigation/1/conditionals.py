...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    # Для однострочного варианта мы можем использовать только else....if и так далее все остальные строки.
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    return (
        "LOW" if status < threshold * 0.9
        else "NORMAL" if threshold * 0.9 <= status <= threshold * 1.1
        else "DANGER"
    )
